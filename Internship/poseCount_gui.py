...
def classifyPose_Easy(landmarks, output_image, count, display=False):
    ...
    cv2.putText(frame, 'Count: {}'.format(int(count)), (10, 150),cv2.FONT_HERSHEY_PLAIN, 2, (0, 255, 0), 3)
...
video = cv2.VideoCapture("C:\\Users\\mayan\\OneDrive\\Desktop\\VS Code\\Internship\\test.mp4")
...
def Input():
    ...
    time1 = 0
    count = 0
    while video.isOpened() and (difficulty=='Easy' or difficulty=='Hard'):
        print(difficulty)
        global ok, frame
        ok, frame = video.read()
        if not ok:
        ...
        time1 = time2
        cv2.imshow(f'{str(Name)} Pose Detection'.format(), frame)
        k = cv2.waitKey(1)
...
b1 = Button(text='Click',command=Input)
b1.pack()
r.mainloop()